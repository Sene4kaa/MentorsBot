...
@router.callback_query(F.data == "CheckQuitWorkshopReasons")
async def start_checking_reasons(callback: CallbackQuery):

    sql = """SELECT * FROM quited_workshops"""
    ...
    if reasons:
        ...
        await callback.message.edit_text(text="Отказов еще нет", reply_markup=get_back_to_admin_menu_kb())
